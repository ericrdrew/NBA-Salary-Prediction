...
Read in 2021-2022 salary dataset from bball ref(only top 500 salaries, TBD getting all 600+), read in stat data
```{r Data}
#https://www.basketball-reference.com/contracts/players.html
salary <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\Salary.csv")
...
#Total Stat data
totalStats <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\totalStats.csv")
...
#Advanced Stats Data
advanced <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\advancedStats.csv")
#Remove the "total" observation(for now)
...
per100 <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\per100.csv")
#Remove the "total" observation(for now)
per100 <- per100 %>% filter(Tm!='TOT')
```

Remove unnecessary variables from each of the datasets before merging
```{r clean}
...
Merge the 4 datasets to create one large dataset with 35 variables, then delete those who didn't play in 2021-22
And those who played less than 
```{r merge}
...
Now to test for collinear variables that we can drop, and also drop 22-23 salary
```{r correlations}
...
Check to make sure all the data looks like it should before modeling
```{r validation}
summary(set1)
...
Set2 Modeling(Subset that removed 1 more variable than set 1)
```{r stepMODEL2}
set2 <- set2 %>% select(-TS.)
...
Perform some diagnostic tests on our forward set1 model from above
```{r diagnostics}
summary(forward)
ggplot(forward, aes(x=fitted.values(forward), y=resid(forward))) + geom_point() + labs(x='Fitted Values', 
                                                                                    y='Residual Values',
...
take turns removing PER, USG., FoulPG, APG
```{r set1TAKE2}
empty.model1 <- lm(data=set1, Sal2122~1)
...
Try interaction effects with the variables kept in our best model(keep those from summary(forward)).
Currently all variables in set3 creates a model with an R2 of .6939
```{r interactions}
summary(forward)
...
Diagnostics with our forward model again(I can't think of anything else to try)
```{r Diagnostics2}
ggplot(forward, aes(x=fitted.values(forward), y=resid(forward))) + geom_point() + labs(x='Fitted Values', 
                                                                                    y='Residual Values',
                                                                                    title='Model1 Resid vs. Fitted')
...
```{r test}
#Total Stat data
testStats <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\testStats.csv", header=TRUE)

testSal <-salary %>% subset(select = c(Player,Sal2223))

#per100 test data
test100 <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\test100Stats.csv")

#advanced data
testAdvanced <- read.csv("C:\\Users\\ericd\\Desktop\\NBA Project\\testAdvanced.csv")
